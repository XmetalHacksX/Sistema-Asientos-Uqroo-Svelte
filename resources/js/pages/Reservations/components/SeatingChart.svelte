<script lang="ts">
    ...
    interface SpaceProps {
        layout_objects?: LayoutObjectProps[];
    }
    ...
    let { 
        ...
        seatingBounds: { x: number; y: number; width: number; height: number } | null;
        distinctRowLetters: Array<{ letter: string; y: number }>;
        onselect: (seat: SeatProps) => void;
    }>();
    ...
    function setupPanZoom(node: HTMLElement, initialKey: string) {
...
>
    <svg width="100%" height="100%" style="display: block;">
        ...
        >
            ...
            <!-- Layout objects (Escenarios, pasillos, etc.) -->
                    ...
                    >
                        <rect
                            ...
                            stroke={selectedSeatId === seat.id
                                ...
                                : 'none'}
                            stroke-width="2"
                            style="transition: fill 0.2s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: {selectedSeatId === seat.id ? '0 0 10px rgba(212,154,21,0.5)' : 'none'};"
                        />
                        <text
                            x={seat.pos_x + 16}
                            y={seat.pos_y + 20}
                            text-anchor="middle"
                            fill={seat.status !== 'disponible' && selectedSeatId !== seat.id ? '#334155' : 'white'}
